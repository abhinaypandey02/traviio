    ...
    return (
      <Container className={'text-center  pb-20 pt-[84px] text-[#140D31] relative'}>
        ...
        <div className="flex justify-between mt-[74px]   gap-4 md:gap-6 w-full ">
          {data?.features?.map((feature, index) => <Feature key={index} locale={locale} data={feature} />)}
    ...
    <section className={'bg-[#F2FAFF] text-center py-3'}>
      <Container>
        ...
        <div className="flex justify-between w-full">
          {data?.features?.map((feature, index) => <SmallFeature locale={locale} key={index} data={feature} />)}
        </div>
      </Container>
    </section>
  )
...
const Feature = ({ data,locale }:  PropsWithLocale<FeatureProps>) => {
  return (
    <div className="relative text-center flex flex-col items-center z-[2]">
      ...
      <p className="opacity-60 max-w-[348px]">
