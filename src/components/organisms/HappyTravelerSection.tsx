// import React from 'react'
import { FreeMode, Navigation } from 'swiper/modules'
import { Swiper, SwiperSlide } from 'swiper/react'

import RatingCard from '../molecule/RatingCard'

import 'swiper/css'
import 'swiper/css/navigation'
const HappyTravelerSection = () => {
  const data = [
    {
      title: 'I Highly Recommended this website',
      star: 5,
      name: 'Deepak Chattwani',
      country:
        'data:image/png;base64,',
      date: '16 Aug,2003',
      review:
        " Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.",
    },
    {
      title: 'I Highly Recommended this website',
      star: 5,
      name: 'Deepak Chattwani',
      country:
        'data:image/png;base64,',
      date: '16 Aug,2003',
      review:
        " Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.",
    },
    {
      title: 'I Highly Recommended this website',
      star: 5,
      name: 'Deepak Chattwani',
      country:
        'data:image/png;base64,',
      date: '16 Aug,2003',
      review:
        " Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.",
    },
    {
      title: 'I Highly Recommended this website',
      star: 5,
      name: 'Deepak Chattwani',
      country:
        'data:image/png;base64,',
      date: '16 Aug,2003',
      review:
        " Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.",
    },
  ]
  return (
    <div className="w-full  px-10 py-10 lg:flex items-center gap-x-10 bg-primary">
      <div className="lg:w-1/4 w-full">
        <h3 className="font-semibold text-3xl">Hear it from our</h3>
        <h3 className="font-semibold text-3xl my-1 text-blue">Happy Travelers</h3>
        <h5 className="text-lg my-2 opacity-60">
          Adored by Countless Explorers of Egypt <br /> Dubai, Saudi Arabia, Turkey, and Israel
        </h5>
        <img
          className="rounded-2xl mt-10"
          src="https://images.unsplash.com/photo-1575986767340-5d17ae767ab0?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8dG91cmlzdHxlbnwwfHwwfHx8MA%3D%3D&w=1000&q=80"
          alt=""
        />
      </div>
      <div className="w-3/4 hidden lg:block">
        <Swiper
          navigation={true}
          slidesPerView={3}
          spaceBetween={30}
          modules={[FreeMode, Navigation]}
          freeMode={true}
          pagination={{
            clickable: true,
          }}
          className="mySwiper"
        >
          {data.map((item, index) => {
            return (
              <SwiperSlide>
                <RatingCard
                  title={item.title}
                  name={item.name}
                  star={item.star}
                  review={item.review}
                  country={item.country}
                  date={item.date}
                  varient={1}
                />
              </SwiperSlide>
            )
          })}
        </Swiper>
      </div>
      <div className="w-full lg:hidden block">
        <Swiper
          navigation={true}
          slidesPerView={1}
          spaceBetween={30}
          modules={[FreeMode, Navigation]}
          freeMode={true}
          pagination={{
            clickable: true,
          }}
          className="mySwiper"
        >
          {data.map((item, index) => {
            return (
              <SwiperSlide key={index}>
                <RatingCard
                  title={item.title}
                  name={item.name}
                  star={item.star}
                  review={item.review}
                  country={item.country}
                  date={item.date}
                  varient={1}
                />
              </SwiperSlide>
            )
          })}
        </Swiper>
      </div>
    </div>
  )
}

export default HappyTravelerSection
